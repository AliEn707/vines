  ...
  module Command
    class Init
      def run(opts)
        ...
        Command::Cert.new.create_cert(domain, File.join(dir, 'config/certs'))
      ...
      def create_directories(dir)
        ...
        File.chmod(0600, File.join(dir, 'config/config.rb'))
      end

      def update_config(domain, dir)
        config = File.expand_path('config/config.rb', dir)
